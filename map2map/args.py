...
def add_train_args(parser):
    ...
    # tamaño físico de la subcaja (Mpc/h)
    parser.add_argument('--L-sub', type=float, default=250.0,
            help='physical size of each sub-box [Mpc/h]')

    # pesos de las distintas pérdidas
    parser.add_argument('--lambda-eul', type=float, default=0.5,
            help='weight for Eulerian MSE loss')
    parser.add_argument('--lambda-lag', type=float, default=0.1,
            help='weight for Lagrangian MSE loss')
    ...
    parser.add_argument('--lambda-spec', type=float, default=0.5,
            help='weight for spectral (Fourier) loss')

    # parámetros específicos del término espectral
    parser.add_argument('--spec-kmin', type=float, default=0.3,
            help='minimum k [h/Mpc] to include in spectral loss')
    parser.add_argument('--spec-alpha', type=float, default=2.0,
            help='exponent in k-weight: w(k) ~ (k/k_nyq)^alpha')
