...
def dist_init(rank, args):
    ...
    NOTE: We do *not* override CUDA_VISIBLE_DEVICES or set_device here
          so that each spawned process already knows which GPU it uses.
    """
    # --- Removed second set_device here; we do it in gpu_worker. --- #

    os.environ["LOCAL_RANK"] = str(rank)   # optional
    os.environ["RANK"] = str(rank)         # optional
    ...
    print(f"✅ Rank {rank} initialized (dist_init)")


def node_worker(args):
    """
    Spawns multiple GPU processes on a single node.
    """
    # Ensure SLURM provides the expected node and GPU environment variables
    args.nodes = int(os.environ.get("SLURM_JOB_NUM_NODES", 1))  # Default to 1 node
    args.gpus_per_node = torch.cuda.device_count()

    if args.gpus_per_node < 1:
        raise RuntimeError(f"🚨 No GPUs found on node! Check CUDA setup.")

    args.world_size = args.nodes * args.gpus_per_node  # Total number of processes

    # Spawn one process per GPU
    spawn(gpu_worker, nprocs=args.gpus_per_node, args=(args,))
...
def gpu_worker(local_rank, args):
    ...
    """
    # ---------------------------------------------------------------- #
    # 1) Pin each spawned process to its own GPU (no environment rewrite).
    # ---------------------------------------------------------------- #
    torch.cuda.set_device(local_rank)
    device = torch.device("cuda", local_rank)

    # Compute global rank if multi-node:
    node = int(os.environ.get("SLURM_NODEID", 0))  # Default to 0 for single-node
    rank = node * args.gpus_per_node + local_rank  # ✅ Compute global rank
    
    # Initialize RNG differently per rank
    ...
    print(f"args.in_chan: {args.in_chan}, type: {type(args.in_chan)}")
    print(f"args.out_chan: {args.out_chan}, type: {type(args.out_chan)}")
...
def train(epoch, loader, model, criterion,
    ...
    # [0]=lag  [1]=eul  [2]=total (lxe)  [3]=grad
    ...
    for i, data in enumerate(loader):
        ...
        # print(f"input shape: {input.shape}, target shape: {target.shape}, output shape: {output.shape}")
        ...
        ################ 
        loss = (
            args.lambda_eul  * eul_loss +
            args.lambda_lag  * lag_loss +
        ...
        epoch_loss[3] += grad_loss.detach()

        optimizer.zero_grad()
        loss.backward()  # NOTE actual loss is log(loss)
        optimizer.step()
        ...
        if batch % args.log_interval == 0:
            ...
            dist.all_reduce(grad_loss)
            lag_loss /= world_size
            eul_loss /= world_size
            loss /= world_size
            grad_loss /= world_size
            if rank == 0:
                ...
                logger.add_scalar('loss/batch/train/lxe', loss.item(),
                                  global_step=batch)
                logger.add_scalar('loss/batch/train/grad', grad_loss.item(),
                                  global_step=batch)
...
def validate(epoch, loader, model, criterion, logger, device, args):
    ...
    if rank == 0:
        ...
        logger.add_scalar('loss/epoch/val/lxe', epoch_loss[2],
                          global_step=epoch+1)
        logger.add_scalar('loss/epoch/val/grad', epoch_loss[3],
                          global_step=epoch+1)
